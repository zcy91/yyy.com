<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:75:"D:\wamp64\www\yyy.com\public/../application/index\view\user\photograph.html";i:1533712018;s:66:"D:\wamp64\www\yyy.com\public/../application/index\view\header.html";i:1536904258;s:66:"D:\wamp64\www\yyy.com\public/../application/index\view\footer.html";i:1533704758;s:65:"D:\wamp64\www\yyy.com\public/../application/index\view\share.html";i:1534127867;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<div style="position:relative;" id="opacResultTable">
    ...
    <div class="m-head">
        ...
        <p>摄影大赛</p>
        <!--<a href="<?php echo url('/index/user/dan_photorelease'); ?>" class="i"><i class='community-add-icon'></i></a>-->
        <!--<a href="<?php echo url('/index/user/zu_photorelease'); ?>" class="j"><i class='community-add-icon'></i></a>-->
    </div>
    <!-- 公共头部结束 -->
    <!-- 评论区 -->
    <div class="sphome-class">
        <ul>
            <li <?php if((!empty($order_bay) && $order_bay==1) or empty($order_bay)): ?> class="ahover" <?php endif; ?>onclick="javascript:location.href='<?php echo url('/index/user/photograph',array('order'=>1)); ?>'">默认</li>
            <li <?php if(!empty($order_bay) && $order_bay==2): ?> class="ahover"  <?php endif; ?>onclick="javascript:location.href='<?php echo url('/index/user/photograph',array('order'=>2)); ?>'">投票↓</li>
            <li <?php if(!empty($order_bay) && $order_bay==3): ?> class="ahover"  <?php endif; ?> onclick="javascript:location.href='<?php echo url('/index/user/photograph',array('order'=>3)); ?>'">投票↑</li>
        </ul>
    </div>
    <?php 
        if(empty($order_bay)){
     ...
     ?>
    <input type="hidden"  value = "<?php echo $order_bay; ?>" id="order_bay">
            ...
            <li>
                <!--<div class='comment-wrapper'>-->
                    <!--<div style="width:2rem;position:absolute;right:-.5rem;top:.1rem;background-color:#fff;height: 1rem;">-->
                        <!--<p onclick="toupiao(<?php echo $vo['id']; ?>)" id="submit" >投票</p>-->
                    <!--</div>-->
                <img class='user-portrait' src="<?php if(!empty($vo['avatar'])) echo imgUrl($vo['avatar']);else echo '/static/home/images/logo.jpg' ?>">
                <div class='right-msg-wrapper'>
                    <p class='user-name'><?php echo $vo['username']; ?></p>
                    <a href="<?php echo url('index/user/content',array('id'=> $vo['id'])); ?>" class="user-msg-detail">
                       <?php echo $vo['content']; ?>

                    </a>
                    <div class="my-gallery" itemscope itemtype="http://schema.org/ImageGallery" data-pswp-uid="<?php echo $key; ?>">
                        ...
                        <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
                                                    <a href="<?php echo $v2; ?>" data-size = <?php echo $vo['size'][$key][0]; ?>x<?php echo $vo['size'][$key][1]; ?>>
                                                        <div class="image-container" style="background-image: url(<?php echo $v2; ?>)" ></div>
                                                        <img src="<?php echo $v2; ?>" style="display: none">
                                                    </a>
                                                </figure>
                ...
                <div class="bottom-info">
                    ...
                    <span class='hours'><?php echo $vo['timestr']; ?></span>
                    <div class='love-wrapper'>
                        <i class="fa fa-heart fa-lg love-icon <?php if($iszan == 1): ?> love_red <?php endif; ?>" value="<?php echo $vo['id']; ?>" aria-hidden="true" onclick="zan(this)"></i>
                        <span class="zan"><?php echo $vo['zan']; ?></span>
                    </div>
                    <div class='msg-count'>
                        <i class="fa fa-commenting fa-lg message-icon" aria-hidden="true" value="<?php echo $vo['id']; ?>"></i><span><?php echo $vo['comment']; ?></span>
                    </div>
                    <div class='vote-wrapper'>
                        <input type="button" value='投票' onclick="toupiao(<?php echo $vo['id']; ?> ,this)" >
                        <span class="span"><?php echo $vo['vote']; ?></span>票
                    </div>
                        <!--<span style="float: right">投票数：<strong style="color: #0aba07"><?php echo $vo['vote']; ?></strong></span>-->
...
<!--<script type="text/javascript" src="//js.users.51.la/19421681.js"></script>-->
    ...
    <script>
        function toupiao(id , that) {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo url('index/user/toupiao'); ?>",
                async:false,
                data: {
                    "id":id
                },
...
//             window.location.href="<?php echo url('index/user/photograph'); ?>";
                }
            });
        }
        function search() {
            var mobile = $("#mobile").val()
            window.location.href="<?php echo url('index/user/photograph'); ?>"+"?mobile="+mobile
        }
    </script>
    <script>
        ...
        function ajaxSourchSubmit() {
            page += 1;
            var order_bay = <?php echo $order_bay; ?>;
            if(!order_bay){
                 order_bay = 1;
            }
            $.ajax({
                type: "GET",
                url: "<?php echo url('index/user/ajaxphotograph'); ?>"+"?p=" + page+"&order_bay="+order_bay,
                beforeSend: function () {
        ...
        $(".message-icon").click(function () {
            ...
            $.ajax({
                type: 'post',
                url: "<?php echo url('index/discovery/iscomment_login'); ?>?mid=-1&aid="+id,
                async:false,
                dataType:'json',
                success: function(data){
                    if(data.status==1){
                   ...
                   // <form action="<?php echo url('index/discovery/comment'); ?>&mid=0&aid='+id+'" enctype="multipart/form-data" method="post">
                        ...
                        $('#comment_sub1').on('click', function(){
                            $.ajax({
                                type: 'post',
                                url: "<?php echo url('index/discovery/comment'); ?>?mid=0&aid=" + id + "&cid=1",
                                async: false,
                                dataType: 'json',
