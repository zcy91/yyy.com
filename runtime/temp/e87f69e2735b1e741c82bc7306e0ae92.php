<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:73:"D:\wamp64\www\yyy.com\public/../application/back\view\article\adlist.html";i:1532921426;}*/ ?>
<!DOCTYPE html>
<html lang="en"><head>
	<meta charset="UTF-8">
	<title>个人信息</title>
		...
		<div class="layui-inline">
                    <div class="layui-input-inline">
                        <select name="search">
                          ...
                          <option value="aid" <?php if(!empty($query['query']['search']) && $query['query']['search']=='aid'): ?>selected=""<?php endif; ?>>标题</option>
                        </select>
                    </div>
		    <div class="layui-input-inline">
                ...
                <div class="layui-inline">
			<a class="layui-btn layui-btn-normal"  onclick="ad_add()">添加内容</a>
		</div>
                ...
                <div class="layui-form news_list">
					...
					<tbody class="news_content">
						...
						<tr id="tr_<?php echo $vo['id']; ?>">
							...
							<td>
                                                            <?php if($vo['type']==1): ?>
                                                            <button class="layui-btn layui-btn-danger layui-btn-mini">轮播</button>
                                                            <?php elseif($vo['type']==2): ?>
                                                            <button class="layui-btn  layui-btn-mini">活动</button>
                                                            <?php elseif($vo['type']==3): ?>
                                                            <button class="layui-btn layui-btn-normal layui-btn-mini">广告</button>
                                                            <?php endif; ?>
                                                        </td>
                                                        <td><?php if(!empty($vo['stime'])) echo date("Y-m-d H:i",$vo['stime']); ?></td>
                                                        <td><?php if(!empty($vo['etime'])) echo date("Y-m-d H:i",$vo['etime']); ?></td>
							<td>
							...
							<td>
                                                            <a class="layui-btn layui-btn-mini" onclick="comment_edit(<?php echo $vo['id']; ?>)"><i class="iconfont icon-edit"></i> 编辑</a>
								<a class="layui-btn layui-btn-danger layui-btn-mini"  onclick="comment_del(<?php echo $vo['id']; ?>)" data-id="1"><i class="layui-icon"></i> 删除</a>
							</td>
						</tr>
...
//删除
function comment_del(id){  //删除
        var _this = $(this);
        layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
                if(index>0){
                    $.ajax({
                    type: 'POST',
                    url: 'addel.html?id='+id,
